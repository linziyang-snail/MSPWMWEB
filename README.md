...
專案保留 `package.json` engines：`^20.19.0 || >=22.12.0`，可涵蓋公司 Node 23.1.0。根目錄 `.nvmrc` 固定為 `23.1.0` 供本機對齊。
...
公司 Jenkins DEV job 目前使用 Node 23.1.0 與 npm 10.9.0，建置與部署流程如下：
...
- build 指令：`npm run build:dev`
- 產物目錄：`dist/`
- Windows Server 靜態檔目錄：`D:/nginx/MSPWMWEB`
- Nginx root：`root D:/nginx/MSPWMWEB;`
- 部署後應是 `D:/nginx/MSPWMWEB/index.html` 與 `D:/nginx/MSPWMWEB/assets/...`，不要變成 `D:/nginx/MSPWMWEB/dist/index.html`。

靜態檔部署不需要執行 nginx reload。Jenkins 目前不執行 `nginx.exe -s reload`，因為 DCUser 對既有 nginx process 發 reload 可能出現 `OpenEvent("Global\ngx_reload_xxxx") failed (5: Access is denied)`。只要沒有修改 `nginx.conf`，更新 `D:/nginx/MSPWMWEB` 靜態檔即可；`nginx.exe -t` 只用來檢查 config，不負責 reload。
...
- timeout：`60000` ms。
- request interceptor 會從 `sessionStorage.getItem("mspwm.accessToken")` 帶 `Authorization: Bearer <token>`。
